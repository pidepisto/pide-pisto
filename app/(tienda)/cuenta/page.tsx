...
const CARD = { backgroundColor: 'oklch(1 0 0)', border: '1px solid oklch(0.90 0.02 75)', borderRadius: '1rem' }
...
export default function CuentaPage() {
  ...
  const SECCIONES: Seccion[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { icon: HeadphonesIcon, label: 'Ayuda',           href: 'https://wa.me/521XXXXXXXXXX?text=Hola%2C%20necesito%20ayuda%20con%20mi%20pedido%20de%20Pide%20Pisto', target: '_blank' },
  ...
  if (!perfil) return (
    <div style={{ backgroundColor: BG, minHeight: '100vh' }}>
      ...
      <div className="flex flex-col items-center py-20 px-8 gap-5">
      ...
      <div className="px-4 flex flex-col gap-5 pb-32 max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-full flex-shrink-0 overflow-hidden relative"
            ...
            {perfil.avatar_url ? (
              <Image src={perfil.avatar_url} alt={perfil.nombre ?? 'Avatar'} fill className="object-cover" sizes="56px" />
            ) : (
              <div className="w-full h-full flex items-center justify-center"
                style={{ backgroundColor: 'oklch(0.50 0.22 24 / 0.12)' }}>
                <span style={{ fontFamily: 'var(--font-bebas)', fontSize: '1.4rem', color: RED }}>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-base font-bold truncate" style={TXT}>{perfil.nombre ?? 'Mi cuenta'}</p>
            <Link href="/cuenta/editar" className="flex items-center gap-1 text-sm font-medium mt-0.5" style={{ color: RED, fontFamily: 'var(--font-dm-sans)' }}>
              <Pencil className="h-3 w-3" /> Editar perfil
            </Link>
          </div>
        </div>

        {/* Accesos rápidos: Pedidos / Ayuda / Métodos de pago */}
        <div className="grid grid-cols-3 gap-3">
          {[
            { icon: Package,         label: 'Pedidos',           href: '/pedidos' },
            { icon: HeadphonesIcon,  label: 'Ayuda',             href: '#' },
            { icon: CreditCard,      label: 'Métodos\nde pago',  href: '#' },
          ].map(({ icon: Icon, label, href }) => (
            <Link key={label} href={href}>
              <div className="rounded-2xl p-4 flex flex-col items-center gap-2 transition-all active:scale-95" style={CARD}>
                <Icon className="h-6 w-6" style={{ color: 'oklch(0.2 0.03 30)', strokeWidth: 1.6 }} />
                <span className="text-xs font-semibold text-center leading-tight whitespace-pre-line" style={TXT}>{label}</span>
              </div>
            </Link>
          ))}
        </div>
              ...
              {pedidos.map((p, i) => (
                <div key={p.id}>
                  ...
                  <Link href={`/pedidos/${p.id}`}>
                    <div className="flex items-center gap-3 px-4 py-3.5 transition-colors active:bg-black/3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                        style={{ backgroundColor: `${estadoColor[p.estado]}15` }}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs" style={{ color: estadoColor[p.estado], fontFamily: 'var(--font-dm-sans)', fontWeight: 500 }}>
                          {estadoLabel[p.estado]}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-bold" style={{ color: RED, fontFamily: 'var(--font-bebas)', fontSize: '1rem', letterSpacing: '0.02em' }}>${p.total.toFixed(0)}</p>
        ...
        {SECCIONES.map((sec) => (
          <div key={sec.titulo}>
            {sec.titulo && (
              <p className="text-base font-bold mb-3" style={TXT}>{sec.titulo}</p>
            )}
            <div className="rounded-2xl overflow-hidden" style={CARD}>
        ...
        <div className="text-center pb-2">
          ...
          <p className="text-xs mt-0.5" style={{ color: 'oklch(0.65 0.02 40)', fontFamily: 'var(--font-dm-sans)' }}>
